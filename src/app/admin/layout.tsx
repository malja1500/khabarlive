...
import {
  LayoutDashboard, Newspaper, PlusCircle, Users, MessageSquare,
  Tag, BarChart2, Settings, Home, LogOut, Menu, X, ChevronRight, Pin,
} from "lucide-react";
...
const menuItems = [
  ...
  { key:"pinned",     href:"/admin/pinned",        icon:Pin },
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoggedIn || user?.role !== "admin") router.replace("/login");
  ...
  const labels: Record<string,string> = {
    ...
    addNews:    t.addNews,
    pinned:     locale==="fa"?"اخبار پین‌شده":locale==="ar"?"الأخبار المثبتة":"Pinned News",
    users:      t.users,
    comments:   t.comments,
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full" style={{ background:"var(--card)" }}>
      <div className="p-5 pb-4 flex items-center justify-between" style={{ borderBottom:"1px solid var(--border)" }}>
        <div>
        ...
        <button className="md:hidden w-7 h-7 rounded-full flex items-center justify-center" style={{ background:"var(--bg3)", color:"var(--text2)" }} onClick={() => setSidebarOpen(false)}>
          <X size={14}/>
        </button>
      </div>
          ...
          return (
            <Link key={key} href={href}>
              <div className={`flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium cursor-pointer transition-all ${isActive?"admin-nav-active":"hover:bg-[--bg3]"}`} style={{ color:isActive?"var(--text)":"var(--text2)" }}>
                ...
                {badge!=null && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{ background:"var(--accent)" }}>{badge}</span>}
      ...
      <div style={{ borderTop:"1px solid var(--border)" }}>
        ...
        <button onClick={() => setShowLogout(true)} className="w-full flex items-center gap-2.5 px-4 py-3 text-xs font-medium hover:bg-[--bg3] transition-all" style={{ color:"var(--text2)" }}>
          <LogOut size={15}/>{locale==="fa"?"خروج":locale==="ar"?"خروج":"Logout"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex min-h-screen" style={{ direction:locale==="en"?"ltr":"rtl" }}>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col w-56 shrink-0 sticky top-0 h-screen overflow-y-auto" style={{ borderInlineEnd:"1px solid var(--border)" }}>
        <SidebarContent/>
      </aside>
      ...
      {/* Mobile drawer */}
      <aside className="fixed top-0 bottom-0 z-50 w-64 flex flex-col md:hidden transition-transform duration-300" style={{ insetInlineStart:0, transform:sidebarOpen?"translateX(0)":locale==="en"?"translateX(-100%)":"translateX(100%)", borderInlineEnd:"1px solid var(--border)" }}>
        <SidebarContent/>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto min-h-screen" style={{ background:"var(--bg)" }}>
        {/* Mobile topbar */}
        <div className="md:hidden sticky top-0 z-30 flex items-center justify-between px-4 h-14 border-b" style={{ background:"var(--card)", borderColor:"var(--border)" }}>
          <button onClick={() => setSidebarOpen(true)} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background:"var(--bg3)", color:"var(--text2)" }}>
            <Menu size={18}/>
          </button>
          <div className="text-sm font-black" style={{ color:"var(--accent)" }}>🔴 {locale==="fa"?"پنل مدیریت":locale==="ar"?"لوحة الإدارة":"Admin"}</div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={user!.avatar||"https://i.pravatar.cc/40"} alt="" className="w-7 h-7 rounded-full object-cover border-2" style={{ borderColor:"var(--accent)" }}/>
        </div>
        {children}
      </main>
