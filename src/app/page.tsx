...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setCounts({ n:Math.round(targets.n*e), r:Math.round(targets.r*e), v:Math.round(targets.v*e) });
  ...
  const heroList = pinned.length > 0
    ? [
        ...pinned.map(id=>news.find(n=>n.id===id)).filter(Boolean) as NewsItem[],
  ...
  const getBg = (cat: string) => CATEGORY_META[cat as keyof typeof CATEGORY_META]?.bg || "#666";
      ...
      <main className="max-w-[1280px] mx-auto px-4 sm:px-6 py-8">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10 animate-fade-up delay-200">
            <Link href={`/news/${heroMain.id}`} className="lg:col-span-2">
              <div className="relative rounded-2xl overflow-hidden h-[380px] sm:h-[460px] cursor-pointer group">
                ...
                <img src={heroMain.image} alt={heroMain.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="eager"/>
                ...
                <div className="absolute bottom-0 p-6 w-full">
                  <div className="cat-badge mb-3" style={{ background:getBg(heroMain.category) }}>
                    {getLabel(heroMain.category)}
                  </div>
                  <h2 className="text-xl sm:text-2xl font-black text-white leading-relaxed mb-3 line-clamp-3">
                    {locale==="en"&&heroMain.titleEn ? heroMain.titleEn : locale==="ar"&&heroMain.titleAr ? heroMain.titleAr : heroMain.title}
                  </h2>
            ...
            <div className="flex flex-col gap-4">
              {heroSide.map(item => (
                <Link key={item.id} href={`/news/${item.id}`}>
                  <div className="card flex overflow-hidden cursor-pointer transition-all hover:border-[--border2] hover:-translate-x-1 group" style={{ border:"1px solid var(--border)", height:"calc((460px - 32px)/3)" }}>
                    <div className="w-28 shrink-0 overflow-hidden">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={item.image} alt="" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy"/>
                    </div>
                    <div className="p-3 flex flex-col justify-between flex-1 min-w-0">
                      <div>
                        <div className="text-[10px] font-bold uppercase mb-1" style={{ color:CATEGORY_META[item.category as keyof typeof CATEGORY_META]?.color||"var(--accent)" }}>
        ...
        <div className="card grid grid-cols-2 sm:grid-cols-4 gap-0 mb-10 animate-fade-up delay-300 text-center overflow-hidden">
          {[
            ...
            <div key={i} className="py-6 px-4" style={{ borderInlineStart:i>0?"1px solid var(--border)":"none" }}>
              <div className="text-2xl sm:text-3xl font-black mb-1" style={{ background:"linear-gradient(135deg,#e63946,#f4a261)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>{s.num}</div>
        ...
        <div className="animate-fade-up delay-400">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/news" className="btn-secondary text-xs py-1.5 px-4 rounded-xl">{t.home.viewAll}</Link>
          </div>

          {/* Category pills */}
          <div className="flex gap-2 mb-6 flex-wrap">
            {(["all","tech","economy","sport","world","politics","culture"] as const).map(cat => (
              <button key={cat} onClick={() => setFilter(cat)} className="px-4 py-2 rounded-full text-xs font-bold transition-all border" style={{ background:filter===cat?"var(--accent)":"var(--card)", color:filter===cat?"#fff":"var(--text2)", borderColor:filter===cat?"var(--accent)":"var(--border)" }}>
                {cat==="all" ? t.categories.all : `${CATEGORY_META[cat].icon} ${CATEGORY_META[cat][locale as keyof typeof CATEGORY_META[typeof cat]] as string}`}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {Array.from({length:8}).map((_,i)=><div key={i} className="card h-72 shimmer"/>)}
            </div>
          ) : filtered.length===0 ? (
            <div className="text-center py-16" style={{ color:"var(--text3)" }}>{t.news.noResults}</div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 group">
              {filtered.slice(0,8).map(item=>(
                <div key={item.id} className="group"><NewsCard news={item}/></div>
              ))}
            </div>
          )}
        </div>
