...
export async function GET() {
  ...
  return NextResponse.json({ categories: await categoryQueries.getAll() });
}
...
export async function POST(req: NextRequest) {
  ...
  if (!key || !labelFa)
    return NextResponse.json({ error: "key and labelFa required" }, { status: 400 });
  if (await categoryQueries.findByKey(key))
    return NextResponse.json({ error: "key_exists" }, { status: 409 });
  const cat: DBCategory = {
    id: "cat-" + Date.now(), key, labelFa,
    labelEn: labelEn||key, labelAr: labelAr||key,
    icon: icon||"📰", color: color||"#666",
    bg: bg||"rgba(102,102,102,0.85)",
    isActive: true, createdAt: new Date().toISOString(),
  };
  await categoryQueries.insert(cat);
