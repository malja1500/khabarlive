        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-10">
          ...
          <div>
            ...
            <p className="text-sm mb-6" style={{ color: "var(--text2)", lineHeight: 1.8 }}>
              {locale === "fa" ? "پلتفرم اخبار لحظه‌ای با تیمی از بهترین روزنامه‌نگاران ایران. متعهد به ارائه اخبار دقیق و بی‌طرف." : locale === "ar" ? "منصة الأخبار الفورية مع فريق من أفضل الصحفيين." : "A live news platform with a team of top journalists, committed to accurate and unbiased reporting."}
            </p>
            <div className="flex gap-2">
              {["𝕏", "📸", "✈", "▶"].map((icon, i) => (
                <button key={i} className="w-9 h-9 rounded-full flex items-center justify-center text-sm transition-all hover:bg-[--accent] hover:text-white" style={{ background: "var(--bg3)", border: "1px solid var(--border)", color: "var(--text2)" }}>
                  {icon}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {Object.entries(t.categories).filter(([k]) => k !== "all").map(([key, label]) => (
                <li key={key}>
                  <Link href={`/news?category=${key}`} className="text-sm transition-colors hover:text-[--accent]" style={{ color: "var(--text2)" }}>
                    {label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {(locale === "fa" ? ["درباره ما","تماس با ما","تبلیغات","خبرنگاران"] : locale === "ar" ? ["من نحن","اتصل بنا","إعلانات","المراسلون"] : ["About Us","Contact","Advertise","Reporters"]).map((item) => (
                <li key={item}>
                  <a href="#" className="text-sm transition-colors hover:text-[--accent]" style={{ color: "var(--text2)" }}>{item}</a>
                </li>
              ))}
            </ul>
          </div>
