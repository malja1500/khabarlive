  ...
  return (
    <>
      ...
      {!isAdmin && (
        <div className="bg-[--accent] overflow-hidden py-2 relative">
          ...
          <div className="flex whitespace-nowrap ticker-animate">
            {[
              "بورس تهران ۲.۳٪ رشد کرد",
              "ایران ۱۲ سهمیه المپیک ۲۰۲۸ کسب کرد",
              "گوگل Gemini Ultra 2 رونمایی کرد",
              "اجلاس بریکس با ۴۸ کشور",
              "توافق‌نامه آب‌وهوایی جنوا امضا شد",
              "بورس تهران ۲.۳٪ رشد کرد",
              "ایران ۱۲ سهمیه المپیک ۲۰۲۸ کسب کرد",
              "گوگل Gemini Ultra 2 رونمایی کرد",
              "اجلاس بریکس با ۴۸ کشور",
              ...
              <span key={i} className="px-8 text-white text-xs font-medium">
                <span className="opacity-60 mx-2 text-[8px]">●</span>
                {item}
              </span>
            ))}
          </div>
      ...
      <nav
        ...
        style={{
          background: "rgba(var(--bg-rgb,10,10,15),0.96)",
        ...
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 flex items-center h-16 gap-4">
            ...
            <div className="hidden md:flex items-center gap-1 overflow-x-auto no-scrollbar flex-1">
              {cats.map((c) => (
                <Link
                  ...
                  onMouseLeave={(e) => { (e.target as HTMLElement).style.background = ""; (e.target as HTMLElement).style.color = "var(--text2)"; }}
                >
                  {c.label}
                </Link>
              ))}
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full flex items-center justify-center transition-all hover:bg-[--bg3]"
              style={{ color: "var(--text2)" }}
            ...
            <div className="relative">
              <button
                ...
                className="w-8 h-8 rounded-full flex items-center justify-content-center transition-all hover:bg-[--bg3]"
                style={{ color: "var(--text2)" }}
              >
              ...
              {langOpen && (
                <div
                  className="absolute top-10 end-0 card py-2 min-w-[130px] shadow-lg z-50 animate-slide-down"
                ...
                >
                  {(["fa", "en", "ar"] as Locale[]).map((l) => (
                    <button
                      ...
                      className="w-full text-start px-4 py-2 text-sm transition-all hover:bg-[--bg3]"
                      style={{ color: locale === l ? "var(--accent)" : "var(--text2)", fontWeight: locale === l ? "700" : "400" }}
                    >
            ...
            {isLoggedIn && user ? (
              <div className="flex items-center gap-2">
                {user.role === "admin" && (
                  <Link href="/admin" className="hidden sm:block btn-primary text-xs py-1.5 px-3 rounded-lg">
                    {t.nav.admin}
                  </Link>
                )}
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="hidden sm:flex items-center gap-1.5 text-xs font-medium transition-all hover:text-[--accent]"
                    style={{ color: "var(--text2)" }}
                  >
                    <LogOut size={14} />
                    {t.nav.logout}
            ...
            {/* Mobile menu */}
            <button
              className="sm:hidden w-8 h-8 flex items-center justify-center rounded-full hover:bg-[--bg3]"
              style={{ color: "var(--text2)" }}
          ...
          <div className="sm:hidden border-t px-4 py-4 flex flex-col gap-3" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
            ...
            <div className="flex gap-2 pt-2 border-t" style={{ borderColor: "var(--border)" }}>
              {isLoggedIn && user ? (
                <>
                  {user.role === "admin" && (
                    <Link href="/admin" onClick={() => setMobileOpen(false)} className="btn-primary text-xs flex-1 text-center py-2">
                      {t.nav.admin}
                    </Link>
                  )}
                  <button onClick={() => { setShowLogout(true); setMobileOpen(false); }} className="btn-secondary text-xs flex-1 py-2">
                    {t.nav.logout}
                  </button>
                </>
              ) : (
